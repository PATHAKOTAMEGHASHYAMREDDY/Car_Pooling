        ...
        <div className="p-6 max-h-[calc(95vh-80px)] overflow-y-auto">
          ...
          <div className="mb-6 p-4 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-xl border border-blue-100 shadow-sm">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="font-bold text-lg text-gray-900 leading-tight">{selectedRide.source}</p>
                  ...
                  <p className="font-bold text-lg text-gray-900 leading-tight">{selectedRide.destination}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-2xl font-bold text-blue-600">₹{selectedRide.pricePerSeat}</p>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-gray-700 font-medium">{selectedRide.rideDate}</span>
              </div>
              <div className="flex items-center space-x-2">
                <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span className="text-gray-700 font-medium">{selectedRide.rideTime}</span>
              </div>
              <div className="flex items-center space-x-2 col-span-2">
                <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                <span className="text-gray-700">Driver: <span className="font-semibold text-gray-900">{selectedRide.driverName}</span></span>
              </div>
            </div>
            
            {/* Enhanced Seat Availability Display */}
            <div className="mt-4 p-3 bg-white bg-opacity-70 rounded-lg">
              <p className="text-xs font-medium text-gray-600 mb-2">Seat Availability</p>
              <div className="flex items-center justify-between text-sm">
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-1">
                    <div className="w-3 h-3 bg-green-500 rounded-full shadow-sm"></div>
                    <span className="font-medium text-green-700">
                      {selectedRide.availableSeatsRemaining || (selectedRide.availableSeats - (selectedRide.bookedSeats || 0))} Available
                    </span>
                  </div>
                  {selectedRide.pendingSeats > 0 && (
                    <div className="flex items-center space-x-1">
                      <div className="w-3 h-3 bg-yellow-500 rounded-full shadow-sm"></div>
                      <span className="font-medium text-yellow-700">{selectedRide.pendingSeats} Pending</span>
                    </div>
                  )}
                  {selectedRide.confirmedSeats > 0 && (
                    <div className="flex items-center space-x-1">
                      <div className="w-3 h-3 bg-red-500 rounded-full shadow-sm"></div>
                      <span className="font-medium text-red-700">{selectedRide.confirmedSeats} Booked</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              {(selectedRide.availableSeatsRemaining || (selectedRide.availableSeats - (selectedRide.bookedSeats || 0))) > 0 ? (
                <select
                  name="seatsBooked"
                ...
                >
                  {[...Array(Math.min(selectedRide.availableSeatsRemaining || (selectedRide.availableSeats - (selectedRide.bookedSeats || 0)), 4))].map((_, i) => (
                ...
                <div className="p-4 bg-red-50 border border-red-200 rounded-xl">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      <p className="text-red-700 text-sm font-semibold">No seats available</p>
                      <p className="text-red-600 text-xs">All seats are booked or pending approval</p>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="flex space-x-3 mt-8 pt-4 border-t border-gray-100">
            ...
            <button
              ...
              disabled={!bookingForm.passengerName || !bookingForm.passengerPhone || (selectedRide.availableSeatsRemaining || (selectedRide.availableSeats - (selectedRide.bookedSeats || 0))) <= 0}
